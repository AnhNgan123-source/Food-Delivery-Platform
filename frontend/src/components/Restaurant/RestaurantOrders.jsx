...
const RestaurantOrders = () => {
    ...
    const getStatusStyle = (status) => {
        const config = {
            ...
            'COMPLETED': { bg: '#F3E5F5', color: '#7B1FA2', text: 'Xong' }
        };
        return config[status] || config['PENDING'];
    };

    // SỬA TẠI ĐÂY: Hàm format lấy đúng thời gian khách đặt đơn từ database
            ...
            <div style={{ flex: '0 0 400px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ background: '#fff', padding: '20px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 800 }}>Đơn hàng</h2>
                    ...
                    {/* GIỮ NGUYÊN CSS TAB CŨ: Đảm bảo không bị xấu như cái ảnh sếp gửi */}
                    <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', whiteSpace: 'nowrap', paddingBottom: '5px' }}>
                        {['ALL', 'PENDING', 'CONFIRMED', 'PREPARING', 'SHIPPING', 'COMPLETED'].map(tab => (
                            <button 
                                ...
                                style={{
                                    padding: '8px 15px', borderRadius: '10px', border: 'none', fontSize: '13px', fontWeight: 'bold',
                                    background: activeTab === tab ? '#28a745' : '#f5f5f5',
                     ...
                     filteredOrders.map(order => {
                        ...
                        return (
                            <div 
                                ...
                                style={{
                                    background: '#fff', padding: '16px', borderRadius: '16px', cursor: 'pointer',
                                    border: isSelected ? '2px solid #28a745' : '2px solid transparent',
                                    boxShadow: isSelected ? '0 10px 20px rgba(40,167,69,0.1)' : '0 2px 8px rgba(0,0,0,0.02)',
                                    transition: '0.3s transform'
                                }}
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                                    {/* HIỂN THỊ THỜI GIAN CHUẨN ĐÂY SẾP */}
                                    <span style={{ fontWeight: 700, fontSize: '15px', color: '#333' }}>
                                        <i className="far fa-clock" style={{marginRight: '6px', color: '#28a745'}}></i>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ fontSize: '13px', color: '#666' }}>{order.items?.length || 0} món ăn</span>
                ...
                {selectedOrder ? (
                    <>
                        <div style={{ padding: '25px', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff' }}>
                            <div>
                                <h1 style={{ margin: 0, fontSize: '22px', fontWeight: 800 }}>Đơn hàng #{(selectedOrder.order_id || selectedOrder.orderId)}</h1>
                                <span style={{ color: '#999', fontSize: '14px' }}>Thanh toán: {selectedOrder.payment_method || selectedOrder.paymentMethod}</span>
                            </div>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '25px' }}>
                            ...
                            <h4 style={{ marginBottom: '15px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <i className="fas fa-utensils" style={{color: '#28a745'}}></i> 
                                DANH SÁCH MÓN ĂN
                            </h4>
                    ...
                    <div style={{ margin: 'auto', textAlign: 'center', color: '#ccc' }}>
                        ...
                        <p>Chọn một đơn hàng để bắt đầu xử lý</p>
                    </div>
                )}
            </div>
        </div>
    ...
    if (status === 'PENDING') return (
        <>
            ...
            <button style={{ ...btnBase, background: '#dc3545', flex: '0 0 140px' }} onClick={() => {
                const reason = window.prompt("Lý do hủy đơn là gì sếp?");
                if (reason) updateStatus(id, 'CANCELLED');
            }}>TỪ CHỐI</button>
        </>
    );
    if (status === 'CONFIRMED') return <button style={{ ...btnBase, background: '#ff9800' }} onClick={() => updateStatus(id, 'PREPARING')}><i className="fas fa-fire"></i> CHUẨN BỊ MÓN</button>;
    if (status === 'PREPARING') return <button style={{ ...btnBase, background: '#007bff' }} onClick={() => updateStatus(id, 'SHIPPING')}><i className="fas fa-motorcycle"></i> GIAO HÀNG</button>;
    
    // THÊM NÚT HOÀN TẤT ĐÂY SẾP
    if (status === 'SHIPPING') return (
        <button style={{ ...btnBase, background: '#28a745' }} onClick={() => updateStatus(id, 'COMPLETED')}>
            <i className="fas fa-check-double"></i> ĐÃ GIAO THÀNH CÔNG
        </button>
    );

    return <div style={{ ...btnBase, background: '#f5f5f5', color: '#999', cursor: 'default' }}>ĐƠN HÀNG ĐÃ HOÀN TẤT</div>;
};
